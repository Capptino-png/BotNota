const { downloadContentFromMessage, relayWAMessage, mentionedJid, processTime, MediaType, Browser, Presence, Mimetype, Browsers, delay, getLastMessageInChat, WA_DEFAULT_EPHEMERAL, generateWAMessageFromContent, proto, logger, makeInMemoryStore, getContentType, INativeFlowMessage, prepareWAMessageMedia } = require('@fadzzzslebew/baileys');

const path = require('path');

/* Módulos */
const {tesseract, File, creds, LoggerB, Boom, AssemblyAI, axios, fs, cheerio, crypto, util, randomBytes, emoji, P, NodeCache, linkfy, request, ms, FileType, os, ffmpeg, fetch, exec, spawn, moment, colors, readline, execSync } = require('./exports.js');
...
setInterval(() => processedMessages.clear(), 30000);
...
const baileys = require('@fadzzzslebew/baileys');
...
// Caminho para o banco de dados (usando path absoluto mais seguro)
const bancoDadosPath = './Banco de dados';
const usuariosPath = './Banco de dados/usuarios_cadastrados.json';

// Criar pasta e arquivo se não existirem
async function inicializarBancoDadoss() {
    try {
        // Criar pasta se não existir
        await fs.ensureDir(bancoDadosPath);
        
        // Criar arquivo se não existir
        if (!await fs.pathExists(usuariosPath)) {
...
// Função para verificar se é primeira mensagem
async function isPrimeiraMensagem(numeroUsuario) {
    try {
        await inicializarBancoDadoss();
        const usuarios = await fs.readJson(usuariosPath);
        return !usuarios.includes(numeroUsuario);
    } catch (error) {
        console.log('Erro ao verificar primeira mensagem:', error);
        return true; // Se houver erro, trata como primeira mensagem
    }
}

// Função para registrar usuário
async function registrarUsuario(numeroUsuario) {
    try {
        await inicializarBancoDadoss();
        const usuarios = await fs.readJson(usuariosPath);
        if (!usuarios.includes(numeroUsuario)) {
            usuarios.push(numeroUsuario);
            await fs.writeJson(usuariosPath, usuarios);
            console.log(`✅ %c[NOVO LEAD]%c Usuário ${numeroUsuario} registrado com sucesso! 🎉`, 
...
// ======= DETECÇÃO AUTOMÁTICA =======
// Cole este código ANTES do switch/case principal, logo após receber a mensagem

// Verificar se é primeira mensagem (apenas para mensagens privadas, não grupos)
if (!isGroup && await isPrimeiraMensagem(sender)) {
    // Registrar usuário
    await registrarUsuario(sender);
    
    // Reagir com emoji de boas-vindas
    await yurizin.sendMessage(from, {react: {text: `🛎️`, key: info.key}});
    
    // Mensagem de boas-vindas com catálogo
    const mensagemBoasVindas = `🖨️ *CARDAPIO BATMAN NOTAS* 🖨️

PRIMEIRA COMPRA ACOMPANHA CURSO DE PREPARO E LAVAGEM

> 🎯 500 NOTAS TESTE - R$:85,00
> 💷 1 MIL EM NOTAS - R$:160,00
> 💷 2 MIL EM NOTAS - R$:300,00
> 💷 3 MIL EM NOTAS - R$:450,00
> 💷 4 MIL EM NOTAS - R$:580,00
> 💷 5 MIL EM NOTAS - R$:720,00

✅ NOTAS SELECIONADAS✅
NECESSÁRIO ATIVAÇÃO COM CAFÉ, LIXA E ESMALTE 

📠 CÉDULAS DISPONÍVEIS DE
10🦜, 20🐒, 50🐆 E 100🐟
 (Não fazemos mais a de 200)

⚠️ ATENÇÃO ⚠️
CARTA REGISTRADA E SEDEX AGORA É POR CONTA DO CLIENTE 🚚

💾 MÍDIAS DAS NOTAS ABAIXO`;
    ...
    if(isButtons) {
        await yurizin.sendMessage(from, {
            ...
            buttons: [
                {
                    buttonId: `${prefix}referencia`,
                    buttonText: {displayText: "⭐﹚𝐑𝐄𝐅𝐄𝐑𝐄̂𝐍𝐂𝐈𝐀𝐒﹙⭐"},
                    type: 1
                },
                {
                    buttonId: `${prefix}qualidade`,
                    buttonText: {displayText: "👀﹚𝐕𝐄𝐑 𝐍𝐎𝐓𝐀𝐒﹙👀"},
                    type: 1
                },
                {
                    buttonId: `${prefix}vercatalogo`,
                    buttonText: {displayText: "🔥﹚𝐏𝐄𝐆𝐀𝐑 𝐍𝐎𝐓𝐀𝐒 𝐉𝐀﹙🔥"},
                    type: 1
                }
            ],
            footer: '🛍️ 𝐒𝐮𝐚 𝐦𝐞𝐥𝐡𝐨𝐫 𝐞𝐱𝐩𝐞𝐫𝐢ê𝐧𝐜𝐢𝐚 𝐝𝐞 𝐜𝐨𝐦𝐩𝐫𝐚',
            headerType: 1
        }, {quoted: selo});
    } else {
        // Versão sem botões
        const mensagemSemBotoes = `${mensagemBoasVindas}

📱 *Digite os comandos:*
• ${prefix}vercatalogo - Ver todos os produtos
• ${prefix}promocoes - Ofertas especiais  
• ${prefix}atendimento - Falar conosco

Digite um comando para começar! 🚀`;

        yurizin.sendMessage(from, {text: mensagemSemBotoes}, {quoted: selo});
    }
    
    // Retornar para não processar outros comandos nesta primeira interação
    return;
}
...
const estadosPath = './Banco de dados/estados_coleta.json';
const enderecosPath = './Banco de dados/enderecos_clientes.json';
const tagsPath = './Banco de dados/tags_endereco.json';

// ======= FUNÇÕES DO SISTEMA DE ENDEREÇOS =======

// Inicializar arquivos de estado
async function inicializarSistemaEndereco() {
    try {
        await fs.ensureDir('./Banco de dados');
        
        if (!await fs.pathExists(estadosPath)) {
            await fs.writeJson(estadosPath, {});
        }
        
        if (!await fs.pathExists(enderecosPath)) {
            await fs.writeJson(enderecosPath, {});
        }
        ...
        console.log('Erro ao inicializar sistema de endereço:', error);
    }
}

// Verificar se usuário está em coleta de endereço
async function isColetandoEndereco(numeroUsuario) {
    try {
        await inicializarSistemaEndereco();
        const estados = await fs.readJson(estadosPath);
        return estados[numeroUsuario] === 'coletando_endereco';
    } catch (error) {
        return false;
    }
}

// Definir estado de coleta
async function setEstadoColeta(numeroUsuario, estado) {
    try {
        await inicializarSistemaEndereco();
        const estados = await fs.readJson(estadosPath);
        
        if (estado) {
            estados[numeroUsuario] = estado;
        } else {
            delete estados[numeroUsuario];
        }
        
        await fs.writeJson(estadosPath, estados);
    } catch (error) {
        console.log('Erro ao definir estado:', error);
    }
}

// Verificar se usuário tem tag de endereço preenchido
async function temTagEndereco(numeroUsuario) {
    try {
        await inicializarSistemaEndereco();
        const tags = await fs.readJson(tagsPath);
        return tags[numeroUsuario] === 'endereco_preenchido';
    } catch (error) {
        return false;
    }
}

// Definir tag de endereço preenchido
async function setTagEndereco(numeroUsuario) {
    try {
        await inicializarSistemaEndereco();
        const tags = await fs.readJson(tagsPath);
        tags[numeroUsuario] = 'endereco_preenchido';
        await fs.writeJson(tagsPath, tags);
    } catch (error) {
        console.log('Erro ao definir tag:', error);
    }
}

// Salvar endereço do cliente com dados reais
async function salvarEndereco(numeroUsuario, dadosEndereco) {
    try {
        await inicializarSistemaEndereco();
        let enderecos = {};
        
        // Ler arquivo existente com tratamento de erro
        try {
            const conteudo = await fs.readFile(enderecosPath, 'utf8');
            if (conteudo.trim()) {
                enderecos = JSON.parse(conteudo);
            }
        } catch (parseError) {
            console.log('Arquivo JSON corrompido, reinicializando...');
            enderecos = {};
        }
        
        // Salvar endereço
        enderecos[numeroUsuario] = {
            endereco: dadosEndereco,
            dataAtualizacao: new Date().toISOString(),
            dataFormatada: new Date().toLocaleString('pt-BR')
        };
        
        await fs.writeJson(enderecosPath, enderecos);
        console.log(`Endereço salvo para ${numeroUsuario}`);
    } catch (error) {
...
// Obter endereço salvo do cliente
async function obterEndereco(numeroUsuario) {
    try {
        await inicializarSistemaEndereco();
        const enderecos = await fs.readJson(enderecosPath);
        return enderecos[numeroUsuario] || null;
    } catch (error) {
        return null;
    }
}

// NOVA FUNÇÃO: Codificar endereço para usar no botão
function codificarEndereco(endereco) {
    return Buffer.from(endereco).toString('base64');
}

// NOVA FUNÇÃO: Decodificar endereço do botão
function decodificarEndereco(enderecocodificado) {
    try {
        return Buffer.from(enderecocodificado, 'base64').toString('utf8');
    } catch (error) {
        return null;
    }
}

// ======= VERIFICAÇÃO DE ESTADO ANTES DO SWITCH =======
// IMPORTANTE: Cole este código DENTRO do handler de mensagens, ANTES do switch

// Verificar se usuário está coletando endereço E se não é um comando
const isComando = body.startsWith(prefix);
const isComandoEndereco = body.startsWith(`${prefix}endereco_correto`) || body === `${prefix}refazer_endereco`;

if (await isColetandoEndereco(sender) && !isComando) {
    // NOVA LÓGICA: Criar botões com o endereço embutido
    const enderecoUsuario = body.trim();
    const enderecoCodificado = codificarEndereco(enderecoUsuario);
    
    await yurizin.sendMessage(from, {react: {text: `📝`, key: info.key}});
    
    const mensagemConfirmacao = `📋 *CONFIRME SEU ENDEREÇO:*

${enderecoUsuario}

✅ Verifique se todos os dados estão corretos antes de confirmar.

📍 *Certifique-se que incluiu:*
• CEP completo
• Rua e número
• Bairro e cidade
• Estado`;
    ...
    if(isButtons) {
        await yurizin.sendMessage(from, {
            ...
            buttons: [
                {
                    buttonId: `${prefix}endereco_correto ${enderecoCodificado}`,
                    buttonText: {displayText: "✅ ENDEREÇO CORRETO"},
                    type: 1
                },
                {
                    buttonId: `${prefix}refazer_endereco`,
                    buttonText: {displayText: "🔄 REFAZER ENDEREÇO"},
                    type: 1
                }
            ],
            footer: '📍 Confirme para acessar o catálogo completo',
            headerType: 1
        }, {quoted: selo});
    } else {
        const mensagemSemBotoes = `${mensagemConfirmacao}

📱 *Opções:*
• ${prefix}endereco_correto ${enderecoCodificado} - Confirmar endereço
• ${prefix}refazer_endereco - Preencher novamente`;

        yurizin.sendMessage(from, {text: mensagemSemBotoes}, {quoted: selo});
    }
    
    return; // Não processar outros comandos
}

// Se está coletando endereço mas enviou um comando QUE NÃO É DO SISTEMA DE ENDEREÇO, cancelar coleta
if (await isColetandoEndereco(sender) && isComando && !isComandoEndereco) {
    await setEstadoColeta(sender, null);
    reply('🔄 *Coleta de endereço cancelada!*\n\n💡 Use `/vercatalogo` para retomar quando quiser cadastrar seu endereço.');
}
...
const clique = {
  externalAdReply: {
    title: '💰 PARCERIA EXCLUSIVA',
    body: '🚀 R$ 50,00 por venda • 2x ao dia • Monetize seu grupo!',
    thumbnailUrl: 'https://i.imgur.com/0HpxKWF.jpg', // Imagem de dinheiro/parceria
    sourceUrl: 'https://wa.me/message/EXEMPLO', // Substitua pelo seu WhatsApp
    mediaType: 1,
    mediaUrl: 'https://wa.me/message/EXEMPLO',
    renderLargerThumbnail: true,
    showAdAttribution: true,
    ctwaClid: 'parceria_bot_2024'
  },
  forwardingScore: 1,
  isForwarded: false,
  quotedMessage: null,
  ...
  forwardedNewsletterMessageInfo: {
    ...
    newsletterName: '💎 Bot Parcerias VIP',
    serverMessageId: Math.floor(Math.random() * 1000000)
  },
  businessMessageForwardInfo: {
    businessOwnerJid: '5511999999999@s.whatsapp.net' // Substitua pelo seu número
  }
};

// Delay helper com variação orgânica
const delay = ms => new Promise(res => setTimeout(res, ms));
const organicDelay = () => Math.floor(Math.random() * (8000 - 3000) + 3000); // 3-8 segundos
const longDelay = () => Math.floor(Math.random() * (15000 - 10000) + 10000); // 10-15 segundos
...
let db = { 
  ...
  groups: { data: {} }, 
  pendentes: {},
  propostas: {} // Para rastrear status das propostas
};

// Criar pasta do banco de dados se não existir (verificação adicional)
try {
  if (!fs.existsSync(bancoDadosPath)) {
    fs.mkdirSync(bancoDadosPath, { recursive: true });
    console.log('📁 Pasta "Banco de dados" criada com sucesso.');
  }
} catch (error) {
  console.error('❌ Erro ao criar pasta do banco de dados:', error);
}
...
function loadDb() {
  if (fs.existsSync(dbFile)) {
    try {
      ...
      db.messages = db.messages || { processed: [] };
      db.groups = db.groups || { data: {} };
      db.groups.data = db.groups.data || {};
      db.pendentes = db.pendentes || {};
      db.propostas = db.propostas || {};
    } catch (e) {
...
// Função para salvar grupo aceito no banco de dados
function salvarGrupoAceito(groupId, groupName, adminId) {
  try {
    ...
    const cleanGroupId = groupId.replace('@g.us', '').replace(/[^a-zA-Z0-9]/g, '_');
    const filePath = path.join(bancoDadosPath, `grupo_${cleanGroupId}.json`);
    fs.writeFileSync(filePath, JSON.stringify(grupoData, null, 2));
    
    // Também salva no DB principal
    if (!db.groups.data[groupId]) {
      db.groups.data[groupId] = {};
    }
    db.groups.data[groupId].propostaAceita = true;
    db.groups.data[groupId].dataAceite = grupoData.dataAceite;
    saveDb();
    
    console.log(`✅ Grupo ${groupName} salvo no banco de dados.`);
    return true;
  } catch (error) {
    console.error('❌ Erro ao salvar grupo no banco de dados:', error);
    return false;
  }
}

// Função para gerar delay orgânico baseado no número de admins
function calcularDelayOrganicoAdmins(totalAdmins, currentIndex) {
  const baseDelay = organicDelay();
  const additionalDelay = currentIndex * Math.floor(Math.random() * 2000 + 1000); // 1-3s adicional por admin
  return baseDelay + additionalDelay;
}

// Notifica admins com proposta de parceria
async function notifyAdminsInGroup(groupId) {
  const groupData = db.groups.data[groupId];
  if (!groupData || groupData.status !== 'aprovado') {
    console.log(`⛔ Grupo ${groupId} não aprovado — pulando.`);
    return;
  }

  try {
    const meta = await yurizin.groupMetadata(groupId);
    const subject = meta.subject || 'Grupo sem nome';
    const admins = meta.participants.filter(p => p.admin).map(p => p.id);

    console.log(`👑 Admins no "${subject}":`, admins);

    if (groupData.notificacaoFinalizada) {
      console.log(`✅ Admins do grupo "${subject}" já foram notificados.`);
      return;
    }

    // Aguarda um tempo antes de começar as notificações
    await delay(longDelay());

    for (let i = 0; i < admins.length; i++) {
      const admin = admins[i];
      const delayTime = calcularDelayOrganicoAdmins(admins.length, i);
      
      if (i > 0) {
        console.log(`⏳ Aguardando ${delayTime}ms antes de notificar próximo admin...`);
        await delay(delayTime);
      }
      
      console.log(`🔔 Notificando admin ${i + 1}/${admins.length}: ${admin}`);
      
      const propostaMsg = `🤝 *Proposta de Parceria Exclusiva*\n\n` +
        `Olá Admin! 👋\n\n` +
        `Temos uma proposta especial para o grupo *${subject}*:\n\n` +
        `💰 *R$ 50,00* por cada venda do nosso bot\n` +
        `📅 Autorização para *2 divulgações por dia*\n` +
        `🚀 Oportunidade de monetizar seu grupo\n\n` +
        `Esta é uma parceria *exclusiva* e *limitada*.\n\n` +
        `O que você decide? 👇`;

      const buttons = [
        {
          buttonId: `aceitar_${groupId}`,
          buttonText: { displayText: '✅ Aceitar Proposta' },
          type: 1
        },
        {
          buttonId: `negar_${groupId}`,
          buttonText: { displayText: '❌ Negar Proposta' },
          type: 1
        }
      ];
      ...
      try {
        await yurizin.sendMessage(admin, buttonMessage);
        
        // Salva o status da proposta
        if (!db.propostas[groupId]) {
          db.propostas[groupId] = {};
        }
        db.propostas[groupId][admin] = {
          status: 'enviada',
          tentativas: 1,
          dataEnvio: new Date().toISOString()
        };
        
        console.log(`✅ Proposta enviada para admin: ${admin}`);
      } catch (err) {
        console.error(`❌ Erro ao enviar proposta para admin ${admin}:`, err);
      }
    }

    // Marca como finalizado
    groupData.notificacaoFinalizada = true;
    saveDb();
    console.log(`✅ Notificações finalizadas para ${groupId}.`);
    
  } catch (err) {
    console.error(`❌ Erro ao notificar admins do grupo ${groupId}:`, err);
  }
}

// Handler para aceitar proposta
async function handleAceitarProposta(groupId, adminId, msg) {
  try {
    // Aguarda um tempo para parecer mais natural
    await delay(organicDelay());
    ...
    if (!admins.includes(adminId)) {
      await yurizin.sendMessage(adminId, {
        text: '❌ Apenas administradores podem aceitar esta proposta.',
        contextInfo: clique
      });
      return;
    }
    ...
    const sucesso = salvarGrupoAceito(groupId, subject, adminId);
    
if (sucesso) {
 // Aguarda mais um pouco antes de enviar confirmação
 await delay(organicDelay());
 
 const successMsg = `🎉 *Proposta Aceita com Sucesso!*\n\n` +
   `Grupo: *${subject}*\n` +
   `Admin: *${adminId.split('@')[0]}*\n\n` +
   `✅ Seu grupo foi cadastrado em nossa base\n` +
   `💰 Você receberá *R$ 50,00* por cada venda\n` +
   `📅 Pode fazer *2 divulgações por dia*\n\n` +
 ...
 // Salva o ID do grupo no banco de dados principal para comandos futuros
 try {
   const gruposAceitosPath = path.join(bancoDadosPath, 'grupos_aceitos.json');
   let gruposAceitos = [];
   
   // Carrega grupos existentes
   if (fs.existsSync(gruposAceitosPath)) {
     const data = fs.readFileSync(gruposAceitosPath, 'utf8');
     gruposAceitos = JSON.parse(data);
   }
   
   // Adiciona novo grupo se não existir
   if (!gruposAceitos.includes(groupId)) {
     gruposAceitos.push(groupId);
     fs.writeFileSync(gruposAceitosPath, JSON.stringify(gruposAceitos, null, 2));
     console.log(`💾 ID do grupo ${groupId} salvo no banco de grupos aceitos.`);
   }
 } catch (error) {
   console.error('❌ Erro ao salvar ID do grupo no banco de dados:', error);
 }
 
 // Atualiza status da proposta
 if (!db.propostas[groupId]) db.propostas[groupId] = {};
 if (!db.propostas[groupId][adminId]) db.propostas[groupId][adminId] = {};
 
 db.propostas[groupId][adminId].status = 'aceita';
 db.propostas[groupId][adminId].dataAceite = new Date().toISOString();
 saveDb();
 
 console.log(`✅ Proposta aceita pelo admin ${adminId} do grupo ${subject}`);
} else {
      await yurizin.sendMessage(adminId, {
    ...
    console.error('❌ Erro ao processar aceite da proposta:', error);
    try {
      await yurizin.sendMessage(adminId, {
...
// Handler para negar proposta (com insistência)
async function handleNegarProposta(groupId, adminId, msg) {
  try {
    const meta = await yurizin.groupMetadata(groupId);
    const subject = meta.subject || 'Grupo sem nome';
    ...
    // Conta tentativas
    if (!db.propostas[groupId]) db.propostas[groupId] = {};
    if (!db.propostas[groupId][adminId]) db.propostas[groupId][adminId] = { tentativas: 0 };
    
    const tentativas = db.propostas[groupId][adminId].tentativas || 1;
    
    if (tentativas === 1) {
      // Primeira negativa - insistir
      await delay(organicDelay());
      
      const insistenciaMsg = `🤔 *Tem certeza?*\n\n` +
        `Esta é uma oportunidade única, admin!\n\n` +
        `Pense bem:\n` +
        `💰 *R$ 50,00* por venda (passivo)\n` +
        `📈 Monetize seu grupo sem esforço\n` +
        `⏰ Apenas *2 posts por dia*\n` +
        `🔥 Parceria exclusiva e limitada\n\n` +
        `Muitos administradores gostariam desta chance...\n\n` +
      ...
      const buttons = [
        ...
        {
          buttonId: `negar_final_${groupId}`,
          buttonText: { displayText: '❌ Negar Definitivamente' },
          type: 1
        }
      ];
      ...
      await yurizin.sendMessage(adminId, buttonMessage);
      
      db.propostas[groupId][adminId].tentativas = 2;
      db.propostas[groupId][adminId].status = 'negada_primeira';
      
      console.log(`🔄 Insistindo com admin ${adminId} do grupo ${subject}`);
      
    } else {
      // Segunda negativa - aceitar e parar
      await yurizin.sendMessage(adminId, {
        text: `😔 Entendemos sua decisão.\n\nCaso mude de ideia, estaremos aqui!\n\n*Boa sorte com seu grupo!* 🍀`,
        contextInfo: clique
      });
      
      db.propostas[groupId][adminId].status = 'negada_final';
      console.log(`❌ Proposta definitivamente negada pelo admin ${adminId}`);
    }
    
    saveDb();
    
  } catch (error) {
    console.error('❌ Erro ao processar negativa da proposta:', error);
  }
}

// Listener para comandos de botão
yurizin.ev.on('messages.upsert', async m => {
  ...
  // Handler para aceitar proposta
  if (text.startsWith(`aceitar_`) && text.includes('@g.us')) {
    const groupId = text.replace(`aceitar_`, '');
    await handleAceitarProposta(groupId, sender, msg);
    return;
  }


  // Handler para negar proposta
  if (text.startsWith(`negar_`) && text.includes('@g.us')) {
    const groupId = text.replace(`negar_`, '').replace('final_', '');
    await handleNegarProposta(groupId, sender, msg);
    return;
  }

  // Handler para negar definitivamente
  if (text.startsWith(`negar_final_`) && text.includes('@g.us')) {
    const groupId = text.replace(`negar_final_`, '');
    await handleNegarProposta(groupId, sender, msg);

 // Aguarda um tempo orgânico antes de apagar o chat
 await delay(organicDelay());
 
 try {
   // Apaga o chat completo (conversa inteira)
   await yurizin.chatModify({ clear: { includeStarred: false } }, sender);
   await yurizin.chatModify({ delete: true }, sender);
   console.log(`🗑️ Chat completo com admin ${sender} apagado após negativa final.`);
 } catch (error) {
   console.error(`❌ Erro ao apagar chat com ${sender}:`, error);
 }
...
// Fallback para grupos privados
async function handlePrivateFallback({ groupId, inviteCode, sender, subject, messageId, msg }) {
  db.pendentes[groupId] = { inviteCode, status: 'aguardando_aprovacao' };
  delete db.groups.data[`temp-${inviteCode}`];
  db.messages.processed.push(messageId);
  saveDb();

  await yurizin.sendMessage(sender, {text: `🔒 Solicitação enviada para entrar no grupo *${subject}*.`});
  await yurizin.sendMessage(sender, { react: { text: '🔒', key: msg.key } });
}

// Processa mensagens contendo convite
yurizin.ev.removeAllListeners('messages.upsert');
const proc = new Set();
...
yurizin.ev.on('messages.upsert', async m => {
  ...
  try {
    ...
    // Processa comandos de botão primeiro
    if (text && (text.includes(`aceitar_`) || text.includes(`negar_`))) {
      const sender = msg.key.remoteJid;
    ...
    // Processa convites
    const code = text?.match(/chat\.whatsapp\.com\/(\w{20,})/)?.[1];
    const sender = msg.key.remoteJid;
    if (!code) return;

    console.log(`🔗 Convite detectado: ${code}`);

    const already = Object.entries(db.groups.data).some(([, g]) => g.inviteCode === code);
    if (already) {
      await yurizin.sendMessage(sender, {text: `⛔ Este convite já foi utilizado.`, contextInfo: clique});
      await yurizin.sendMessage(sender, { react: { text: '⛔', key: msg.key } });
      db.messages.processed.push(id);
      saveDb();
      return;
    }

    const tempKey = `temp-${code}`;
    db.groups.data[tempKey] = { inviteCode: code };
    saveDb();

    let info;
    try {
      info = await yurizin.groupGetInviteInfo(code);
    } catch {
      delete db.groups.data[tempKey];
      saveDb();
      return;
    }
    ...
    try {
      joined = await yurizin.groupAcceptInvite(code);
    } catch (e) {
      const msgE = e?.message || '';
      const isPrivate = msgE.includes('requires approval') || msgE.includes('not-authorized') || e?.status === 403;
      const isConflict = e?.data === 409 || msgE.includes('conflict');

      if (isPrivate) {
        await handlePrivateFallback({ groupId: info.id, inviteCode: code, sender, subject, messageId: id, msg });
        return;
      }

      if (isConflict) {
        db.groups.data[info.id] = {
          inviteCode: code,
          status: 'aprovado'
        };
        delete db.groups.data[tempKey];
        ...
        await yurizin.sendMessage(sender, {text: `🔓 Bot já está no grupo *${subject}* e foi ativado.`, contextInfo: clique});
        await yurizin.sendMessage(sender, { react: { text: '🔓', key: msg.key } });
        return;
      }
    ...
    if (!joined?.endsWith('@g.us')) {
      await handlePrivateFallback({ groupId: info.id, inviteCode: code, sender, subject, messageId: id, msg });
      return;
    }

    // Entrada bem-sucedida em grupo público
    db.groups.data[joined] = {
      inviteCode: code,
      status: 'aprovado'
    };
    delete db.groups.data[tempKey];
    ...
    await yurizin.sendMessage(sender, {text: `🔓 Bot entrou no grupo público *${subject}*.`, contextInfo: clique});
    await yurizin.sendMessage(sender, { react: { text: '🔓', key: msg.key } });

  } catch (e) {
    console.error('❌ Erro no processamento de convite:', e);
  } finally {
    proc.delete(id);
  }
});

// ✅ Controle para entradas únicas
if (!global._gruposEntradas) global._gruposEntradas = new Set();
if (!global._gruposIgnoradosLog) global._gruposIgnoradosLog = new Set();
if (!global._debounceChats) global._debounceChats = {};

yurizin.ev.on('chats.upsert', chats => {
  if (!Array.isArray(chats)) {
    console.warn('⚠️ chats.upsert inválido', chats);
    return;
  }
  ...
  for (const chat of chats) {
    ...
    // ⏱️ Debounce para evitar múltiplas execuções seguidas
    const now = Date.now();
    if (global._debounceChats[groupId] && now - global._debounceChats[groupId] < 5000) {
      if (!global._gruposIgnoradosLog.has(groupId)) {
        global._gruposIgnoradosLog.add(groupId);
      }
      continue;
    }
    global._debounceChats[groupId] = now;

    if (global._gruposEntradas.has(groupId)) {
      if (!global._gruposIgnoradosLog.has(groupId)) {
        console.log(`🔁 [chats.upsert] Ignorando repetida: ${groupId}`);
        global._gruposIgnoradosLog.add(groupId);
      }
      continue;
    }
    global._gruposEntradas.add(groupId);

    const grupo = db.groups.data[groupId] || {};
    if (grupo.notificacaoFinalizada) {
      console.log(`✅ Notificação já finalizada: ${groupId}`);
      continue;
    }

    console.log(`🎉 [chats.upsert] Bot entrou no grupo: ${groupId}`);

    // Atualiza DB
    db.groups.data[groupId] = {
      inviteCode: grupo.inviteCode || 'desconhecido',
      status: 'aprovado',
      notificacaoFinalizada: false,
      adminsNotificados: grupo.adminsNotificados || {}
    };
    delete db.pendentes[groupId];
    saveDb();

    (async () => {
      await delay(longDelay()); // Delay inicial mais longo
      await notifyAdminsInGroup(groupId);
    })();
  }
});
...
const { handle1CentCommand, handleStatusCommand } = require('./pixSystem');
const EfiPay = require('sdk-node-apis-efi');


// Configuração das credenciais EFI
const efipay = new EfiPay({
    client_id: 'Client_Id_e624f1b7b55dc659db246a62991c71d9189f07db',
    client_secret: 'Client_Secret_7065dc9537c226d88107ad2064153be653dcf562',
    certificate: './PixAutoCert.p12',
    sandbox: false // mude para true se estiver em ambiente de testes
});

// Produtos disponíveis
const produtos = [
    { id: 1, nome: "Produto 1", preco: 85.00 },
    { id: 2, nome: "Produto 2", preco: 160.00 },
    { id: 3, nome: "Produto 3", preco: 300.00 },
    { id: 4, nome: "Produto 4", preco: 450.00 },
    { id: 5, nome: "Produto 5", preco: 580.00 },
    { id: 6, nome: "Produto 6", preco: 720.00 }
];

// Função para gerar valor de frete randômico
function gerarValorFrete() {
    return Math.floor(Math.random() * (15 - 7 + 1)) + 7;
}

// Função para gerenciar frete do usuário
function gerenciarFrete(userId) {
    const dbPath = path.join(__dirname, 'Banco de dados');
    const freteFile = path.join(dbPath, 'valor_frete.json');
    ...
    if (fs.existsSync(freteFile)) {
        try {
            const data = fs.readFileSync(freteFile, 'utf8');
            dadosFrete = JSON.parse(data);
        } catch (error) {
            console.error('Erro ao ler arquivo de frete:', error);
        }
    }
    
    if (!dadosFrete[userId]) {
        dadosFrete[userId] = gerarValorFrete();
        
        try {
            fs.writeFileSync(freteFile, JSON.stringify(dadosFrete, null, 2));
        } catch (error) {
            console.error('Erro ao salvar frete:', error);
        }
    }
    
    return dadosFrete[userId];
}

// Função para gerar PIX de 1 centavo
async function gerarPixUmCentavo() {
    try {
        // Gerar txid único
        ...
        const body = {
            ...
            solicitacaoPagador: 'Pagamento de teste - 1 centavo'
        };
        // Criar cobrança PIX - sintaxe correta: params, body
        const params = {
            txid: txid
        };
       
        const cobranca = await efipay.pixCreateImmediateCharge(params, body);
        ...
        const qrcode = await efipay.pixGenerateQRCode(qrcodeParams);
...
// Função para gerar PIX com valor personalizado
async function gerarPixProduto(valorTotal, nomeProduto) {
    try {
        const txid = Date.now().toString() + Math.random().toString(36).substring(7);
        ...
        const body = {
            ...
            chave: 'pixcaixinhadebrigadeiro@gmail.com',
            solicitacaoPagador: `Pagamento - ${nomeProduto}`
        };
...
function getNextNumber(folderPath, prefix) {
    ...
    // Filtra apenas os arquivos que começam com o prefixo
    const videoFiles = files.filter(file => 
        file.startsWith(prefix) && file.match(/\d+\.mp4$/)
    );
    ...
    // Extrai os números dos arquivos
    const numbers = videoFiles.map(file => {
        const match = file.match(/(\d+)\.mp4$/);
        return match ? parseInt(match[1]) : 0;
    });
    
    // Retorna o maior número + 1
    return Math.max(...numbers) + 1;
}

// Função para salvar o vídeo
async function saveVideo(media, folderPath, prefix) {
    try {
        // Cria a pasta se não existir
        if (!fs.existsSync(folderPath)) {
            fs.mkdirSync(folderPath, { recursive: true });
        ...
        const nextNumber = getNextNumber(folderPath, prefix);
        
        // Formata o número com dois dígitos (01, 02, 03...)
        const fileName = `${prefix}${nextNumber.toString().padStart(2, '0')}.mp4`;
...
case 'test':
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/foto-batman.jpg';
        let thumbnail;
        
        if (fs.existsSync(fotoPath)) {
            thumbnail = fs.readFileSync(fotoPath);
        }
        
        await yurizin.sendMessage(from, {
            text: '🧪 *TESTE DE EXTERNALADREPLY*\n\n✅ Título em negrito\n✅ Descrição (body)\n✅ Miniatura grande\n✅ Link clicável\n\n🦇 Batman BOT ativo!',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'Clique aqui para testar o link',
                    thumbnail: thumbnail,
        ...
        await yurizin.sendMessage(from, { text: '❌ Erro: ' + error.message });
    }
    break;
...
case 'test1':
    // BÁSICO - Só título e body
    await yurizin.sendMessage(from, {
        text: '🧪 Test 1: Básico (sem imagem)',
        contextInfo: {
            externalAdReply: {
                title: 'Título Simples',
                body: 'Descrição básica',
...
case 'test2':
    ...
    await yurizin.sendMessage(from, {
        text: '🧪 Test 2: Com imagem externa (URL)',
        contextInfo: {
            externalAdReply: {
                title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                body: 'Imagem vinda de URL externa',
                thumbnailUrl: 'https://pix.sejaefi.com.br/cob/pagar/0c0d49c44b91431a92e97186fbc15d36',
                sourceUrl: 'https://github.com',
                mediaType: 1
            }
        }
...
case 'test3':
    // COM IMAGEM LOCAL (BUFFER)
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/foto-batman.jpg';
        const thumbnail = fs.readFileSync(fotoPath);
        
        await yurizin.sendMessage(from, {
            text: '🧪 Test 3: Com imagem local (Buffer)',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'Imagem carregada do Android',
                    thumbnail: thumbnail,
                    sourceUrl: 'https://github.com',
                    mediaType: 1
                }
...
case 'test4':
    ...
    try {
        ...
        await yurizin.sendMessage(from, {
            text: '🧪 Test 4: Miniatura GRANDE (renderLargerThumbnail)',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'Imagem em destaque',
                    thumbnail: thumbnail,
                    sourceUrl: 'https://github.com',
                    mediaType: 1,
                    renderLargerThumbnail: true
...
case 'test5':
    // TIPO VÍDEO (mediaType: 2)
    await yurizin.sendMessage(from, {
        text: '🧪 Test 5: Tipo VÍDEO (mediaType: 2)',
        contextInfo: {
            externalAdReply: {
                title: '🎬 Vídeo do Batman',
                body: 'Clique para assistir',
...
case 'test6':
    // COM MEDIA URL ALTERNATIVA
    await yurizin.sendMessage(from, {
        text: '🧪 Test 6: Com mediaUrl alternativa',
        contextInfo: {
            externalAdReply: {
                title: 'Teste com MediaUrl',
                body: 'URL alternativa de mídia',
                thumbnailUrl: 'https://i.imgur.com/batman.jpg',
                mediaUrl: 'https://youtube.com/watch?v=VIDEO',
                sourceUrl: 'https://github.com',
                mediaType: 1
            }
        }
...
case 'test7':
    // SEM LINK (sourceUrl vazio)
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/foto-batman.jpg';
        const thumbnail = fs.readFileSync(fotoPath);
        
        await yurizin.sendMessage(from, {
            text: '🧪 Test 7: Sem link clicável',
            contextInfo: {
                externalAdReply: {
                    title: 'Apenas Visual',
                    body: 'Não leva a lugar nenhum',
                    thumbnail: thumbnail,
                    mediaType: 1
                }
            }
...
case 'test8':
    ...
    try {
        ...
        await yurizin.sendMessage(from, {
            text: '🧪 Test 8: Descrição longa no body',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'Esta é uma descrição bem longa para testar como o WhatsApp renderiza textos extensos no body do externalAdReply',
                    thumbnail: thumbnail,
                    sourceUrl: 'https://github.com',
                    mediaType: 1,
                    renderLargerThumbnail: true
...
case 'test9':
    // COM EMOJIS NO TÍTULO
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/foto-batman.jpg';
        const thumbnail = fs.readFileSync(fotoPath);
        
        await yurizin.sendMessage(from, {
            text: '🧪 Test 9: Título com emojis',
            contextInfo: {
                externalAdReply: {
                    title: '🦇 𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓 ⚡',
                    body: '✨ O melhor bot do WhatsApp 🚀',
                    thumbnail: thumbnail,
                    sourceUrl: 'https://github.com',
                    mediaType: 1,
                    renderLargerThumbnail: true
...
case 'test10':
    // MENSAGEM LONGA + EXTERNALADREPLY
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/foto-batman.jpg';
        const thumbnail = fs.readFileSync(fotoPath);
        
        await yurizin.sendMessage(from, {
            text: '🧪 *Test 10: Mensagem Completa*\n\n' +
                  '📋 Este é um teste com:\n' +
                  '✅ Texto longo e formatado\n' +
                  '✅ ExternalAdReply completo\n' +
                  '✅ Miniatura grande\n' +
                  '✅ Link funcional\n\n' +
                  '🦇 Batman BOT em ação!',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓 - Catálogo',
                    body: 'Clique para ver todos os produtos',
                    thumbnail: thumbnail,
                    sourceUrl: 'https://seusite.com/catalogo',
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
            }
...
case 'test11':
    // MÚLTIPLAS PROPRIEDADES (TESTE COMPLETO)
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/foto-batman.jpg';
        const thumbnail = fs.readFileSync(fotoPath);
        
        await yurizin.sendMessage(from, {
            text: '🧪 Test 11: TODAS as propriedades',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'Teste com todas as propriedades',
                    thumbnail: thumbnail,
                    thumbnailUrl: 'https://pix.sejaefi.com.br/cob/pagar/0c0d49c44b91431a92e97186fbc15d36',
...
case 'test12':
    ...
    try {
        ...
        await yurizin.sendMessage(from, {
            text: '🧪 Test 12: Sem descrição (body)',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    thumbnail: thumbnail,
...
case 'test13':
    // MINIATURA PEQUENA (renderLargerThumbnail: false)
    try {
        const fotoPath = '/storage/emulated/0/notabot/Banco de dados/pix.jpg';
        const thumbnail = fs.readFileSync(fotoPath);
        
        await yurizin.sendMessage(from, {
            text: 'Teste de pagamento 13: renderLargerThumbnail: false\nbody: true\ntitle: true\nexternalAdReply: completo e funcional',
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'clique aqui e efetue o pagamento',
                    thumbnail: thumbnail,
                    thumbnailUrl: 'https://pix.sejaefi.com.br/cob/pagar/0c0d49c44b91431a92e97186fbc15d36',
                    mediaUrl: 'https://youtube.com/video',
                    sourceUrl: 'https://github.com',
...
case 'ativar':
    try {
        ...
        if (!fs.existsSync(fotoPath)) {
            await yurizin.sendMessage(from, { text: '❌ Foto não encontrada!' });
            return;
        }
        
        await yurizin.updateProfilePicture(yurizin.user.id, { url: fotoPath });
        await yurizin.updateProfileName('𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓');
        
        await yurizin.sendMessage(from, { text: '🦇 *Batman ativado!*\n\n✅ Foto atualizada\n✅ Nome atualizada' });
        
    } catch (error) {
        console.error('❌ Erro ao ativar:', error);
        await yurizin.sendMessage(from, { text: '❌ Erro ao ativar Batman!' });
    }
    break;
...
case 'timerstats':
    ...
    const stats = obterEstatisticasTimers();
    let statsText = `📊 *ESTATÍSTICAS DO SISTEMA DE TIMERS*\n\n`;
    statsText += `👥 Sessões em memória: ${stats.totalSessoesMemoria}\n`;
    statsText += `💾 Sessões no banco: ${stats.totalSessoesDB}\n`;
    statsText += `⏱️ Timers rodando: ${stats.comTimerAtivo}\n`;
    statsText += `📨 Alertas enviados: ${stats.alertasEnviados}\n\n`;
    statsText += `🕐 *Top 5 Sessões Mais Longas:*\n`;
    
    stats.sessoesMaisAntigas.forEach((s, i) => {
        statsText += `${i + 1}. ${s.userId} - ${s.duracao}min (${s.mensagens} msgs)\n`;
    });
    
    await yurizin.sendMessage(from, { text: statsText });
    break;
...
case 'limparsessoes':
    ...
    timersInatividade.forEach((data, userId) => {
        if (data.timer) clearTimeout(data.timer);
        removerUsuarioDB(userId);
    });
    timersInatividade.clear();
    
    await yurizin.sendMessage(from, {
        text: '🧹 *Todas as sessões foram limpas!*\n\n✅ Memória e banco de dados resetados.'
    });
    break;
...
case 'adddrop':
    try {
        // Verifica se há mensagem quotada (vídeo selecionado)
        if (!info.message.extendedTextMessage?.contextInfo?.quotedMessage) {
            await yurizin.sendMessage(from, {
                text: '❌ *Marque um vídeo para usar este comando!*\n\n💡 Responda/marque o vídeo e digite: `' + prefix + 'adddrop`'
            });
            break;
        }
        ...
        // Verifica se a mensagem quotada é um vídeo
        if (!quotedMessage.videoMessage) {
            await yurizin.sendMessage(from, {
                text: '❌ *A mensagem selecionada não é um vídeo!*'
            });
            break;
        }

        // React de processamento
        await yurizin.sendMessage(from, {
            react: { text: '⏳', key: info.key }
        });

        // Baixa o vídeo usando downloadContentFromMessage
        ...
        // Salva o vídeo no caminho correto
        const result = await saveVideo(buffer, '/storage/emulated/0/notabot/Banco de dados/Drop/', 'Drop');
        ...
        if (result.success) {
            ...
            console.log(`📹 DROP - Vídeo salvo: ${result.fileName} por ${sender.replace('@s.whatsapp.net', '')} em ${new Date().toLocaleString('pt-BR')}`);
        } else {
            // React de erro
            await yurizin.sendMessage(from, {
                react: { text: '❌', key: info.key }
            ...
            await yurizin.sendMessage(from, {
                text: `❌ *Erro ao salvar vídeo*\n\n🔍 **Detalhes:** ${result.error}`
            });
        }

    } catch (error) {
        console.error('❌ Erro no comando addDrop:', error);
...
case 'addquali':
    try {
        ...
        if (!info.message.extendedTextMessage?.contextInfo?.quotedMessage) {
            await yurizin.sendMessage(from, {
                text: '❌ *Marque um vídeo para usar este comando!*\n\n💡 Responda/marque o vídeo e digite: `' + prefix + 'addquali`'
            });
            break;
        }
        ...
        // Salva o vídeo no caminho correto
        const result = await saveVideo(buffer, '/storage/emulated/0/notabot/Banco de dados/Qualidade/', 'Qualidade');
        ...
        if (result.success) {
            ...
            console.log(`📹 QUALIDADE - Vídeo salvo: ${result.fileName} por ${sender.replace('@s.whatsapp.net', '')} em ${new Date().toLocaleString('pt-BR')}`);
        } else {
            // React de erro
            await yurizin.sendMessage(from, {
                react: { text: '❌', key: info.key }
        ...
        console.error('❌ Erro no comando addQualidade:', error);
...
case "enviar_msg":
    try {
        // Comando liberado para todos os usuários

        // Verifica se há argumentos
        if (args.length < 2) {
            reply(`📞 *Comando ENVIAR*

*Uso:*
• ${prefix}enviar 5511999999999 Sua mensagem aqui
• ${prefix}enviar 5511999999999 (respondendo uma mensagem)

*Exemplos:*
• ${prefix}enviar 5511987654321 Olá! Como você está?
• ${prefix}enviar 5511987654321 (responda uma mensagem para reencaminhar)
...
⚠️ *Use apenas números com DDD e código do país (55)*`);
            break;
        }

        // Extrai o número
        let numeroDestino = args[0];
        
        // Remove caracteres especiais e espaços
        numeroDestino = numeroDestino.replace(/[^0-9]/g, '');
        
        // Validação básica do número
        if (numeroDestino.length < 10 || numeroDestino.length > 15) {
            reply("❌ *Número inválido!* Use o formato: 5511999999999 (com código do país e DDD)");
            break;
        }

        // Adiciona @s.whatsapp.net se não tiver
        if (!numeroDestino.includes('@')) {
            numeroDestino = numeroDestino + '@s.whatsapp.net';
        }

        // Verifica se o número existe no WhatsApp
        let numeroExiste;
        try {
            numeroExiste = await yurizin.onWhatsApp(numeroDestino);
            if (!numeroExiste || numeroExiste.length === 0) {
                reply(`❌ *Número não encontrado no WhatsApp:* ${numeroDestino.replace('@s.whatsapp.net', '')}`);
                break;
            }
        } catch (error) {
            console.log('⚠️ Erro ao verificar número:', error.message);
        }
        ...
        if (info.quoted) {
            console.log('📝 Detectada mensagem respondida para reencaminhamento');
            
            const quotedMsg = info.quoted;
            
            // Verifica o tipo da mensagem respondida
            if (quotedMsg.text) {
                // Mensagem de texto
                mensagemEnviar = quotedMsg.text;
                tipoMensagem = 'texto';
            } else if (quotedMsg.imageMessage) {
                // Imagem
                tipoMensagem = 'imagem';
                mensagemEnviar = quotedMsg.imageMessage.caption || '';
                try {
                    mediaBuffer = await downloadContentFromMessage(quotedMsg, 'image');
                    nomeArquivo = 'imagem.jpg';
                } catch (e) {
                    reply('❌ Erro ao baixar imagem: ' + e.message);
                    break;
                }
            } else if (quotedMsg.videoMessage) {
                // Vídeo
                tipoMensagem = 'video';
                mensagemEnviar = quotedMsg.videoMessage.caption || '';
                try {
                    mediaBuffer = await downloadContentFromMessage(quotedMsg, 'video');
                    nomeArquivo = 'video.mp4';
                } catch (e) {
                    reply('❌ Erro ao baixar vídeo: ' + e.message);
                    break;
                }
                ...
                tipoMensagem = 'audio';
                try {
                    mediaBuffer = await downloadContentFromMessage(quotedMsg, 'audio');
                    nomeArquivo = 'audio.ogg';
                } catch (e) {
                    reply('❌ Erro ao baixar áudio: ' + e.message);
                    break;
                }
            } else if (quotedMsg.documentMessage) {
                // Documento
                tipoMensagem = 'documento';
                mensagemEnviar = quotedMsg.documentMessage.caption || '';
                nomeArquivo = quotedMsg.documentMessage.fileName || 'documento';
                try {
                    mediaBuffer = await downloadContentFromMessage(quotedMsg, 'document');
                } catch (e) {
                    reply('❌ Erro ao baixar documento: ' + e.message);
                    break;
                }
                ...
                try {
                    mediaBuffer = await downloadContentFromMessage(quotedMsg, 'sticker');
                    nomeArquivo = 'sticker.webp';
                } catch (e) {
                    reply('❌ Erro ao baixar sticker: ' + e.message);
                    break;
                }
            } else {
                reply('❌ Tipo de mensagem não suportado para reencaminhamento.');
                break;
            }
        } else {
            // Mensagem de texto normal
            mensagemEnviar = args.slice(1).join(' ');
            
            if (!mensagemEnviar.trim()) {
                reply('❌ *Mensagem vazia!* Digite o texto que deseja enviar ou responda uma mensagem.');
                break;
            }
        ...
        // Log da operação
        console.log(`📤 ENVIO MANUAL: ${pushname} enviando ${tipoMensagem} para ${numeroDestino.replace('@s.whatsapp.net', '')}`);
        ...
        switch (tipoMensagem) {
            case 'texto':
                resultadoEnvio = await yurizin.sendMessage(numeroDestino, {
                    text: mensagemEnviar,
                    footer: `📤 Enviado por: ${pushname}\n🕐 ${new Date().toLocaleString('pt-BR')}`
                });
                break;
                
            case 'imagem':
                const bufferImg = Buffer.concat([mediaBuffer]);
                resultadoEnvio = await yurizin.sendMessage(numeroDestino, {
                    image: bufferImg,
                    caption: `${mensagemEnviar}\n\n📤 Enviado por: ${pushname}\n🕐 ${new Date().toLocaleString('pt-BR')}`
                });
                break;
                
            case 'video':
                const bufferVid = Buffer.concat([mediaBuffer]);
                resultadoEnvio = await yurizin.sendMessage(numeroDestino, {
                    video: bufferVid,
                    caption: `${mensagemEnviar}\n\n📤 Enviado por: ${pushname}\n🕐 ${new Date().toLocaleString('pt-BR')}`
                });
            ...
            case 'audio':
                const bufferAud = Buffer.concat([mediaBuffer]);
                resultadoEnvio = await yurizin.sendMessage(numeroDestino, {
                    audio: bufferAud,
            ...
            case 'documento':
                const bufferDoc = Buffer.concat([mediaBuffer]);
                resultadoEnvio = await yurizin.sendMessage(numeroDestino, {
                    document: bufferDoc,
            ...
            case 'sticker':
                const bufferStk = Buffer.concat([mediaBuffer]);
                resultadoEnvio = await yurizin.sendMessage(numeroDestino, {
                    sticker: bufferStk
                });
                break;
            ...
            default:
                reply('❌ Tipo de mensagem não reconhecido.');
                break;
        }

        // Confirmação do envio
        if (resultadoEnvio) {
            const numeroLimpo = numeroDestino.replace('@s.whatsapp.net', '');
            
            // Tenta obter o nome do contato
            let nomeContato = numeroLimpo;
            try {
                const contactInfo = await yurizin.onWhatsApp(numeroDestino);
                if (contactInfo && contactInfo[0] && contactInfo[0].notify) {
                    nomeContato = contactInfo[0].notify;
                }
            } catch (e) {
                // Usa o número se não conseguir o nome
            }

            reply(`✅ *MENSAGEM ENVIADA COM SUCESSO!*

📞 *Destinatário:* ${nomeContato} (${numeroLimpo})
📝 *Tipo:* ${tipoMensagem.charAt(0).toUpperCase() + tipoMensagem.slice(1)}
👤 *Enviado por:* ${pushname}
🕐 *Horário:* ${new Date().toLocaleString('pt-BR')}

${tipoMensagem === 'texto' ? `💬 *Conteúdo:* "${mensagemEnviar.substring(0, 100)}${mensagemEnviar.length > 100 ? '...' : ''}"` : ''}`);

            // Confirmação do envio simplificada

        } else {
            reply('❌ *Falha no envio!* Tente novamente em alguns segundos.');
        }

    } catch (error) {
        console.error('❌ Erro no comando enviar:', error);
  ...
  case 'enviarcat':
    try {
        ...
        // Verifica se há mensagem quotada (contato selecionado)
        if (info.message.extendedTextMessage?.contextInfo?.quotedMessage) {
            // Pega o remetente da mensagem quotada
            numeroDestino = info.message.extendedTextMessage.contextInfo.participant || 
                           info.message.extendedTextMessage.contextInfo.remoteJid;
            
            console.log(`📞 Contato selecionado: ${numeroDestino}`);
            
        } else {
            // Verifica se foi passado número como parâmetro
            const args = body.split(' ');
            if (args.length < 2) {
                await yurizin.sendMessage(from, {
                    text: `❓ *Como usar o comando:*\n\n*📱 Método 1 - Por número:*\n\`${prefix}enviarcat 5511999999999\`\n\n*👤 Método 2 - Por contato:*\n• Selecione uma mensagem do usuário\n• Digite: \`${prefix}enviarcat\`\n\n💡 *Tip:* Responda uma mensagem da pessoa para enviar diretamente!`
                });
                break;
            }

            numeroDestino = args[1];
            ...
            if (numeroDestino.replace('@s.whatsapp.net', '').length < 10) {
                await yurizin.sendMessage(from, {
            ...
            console.log(`📞 Número digitado: ${numeroDestino}`);
        }

        // Verifica se conseguiu obter um destinatário
        if (!numeroDestino) {
            await yurizin.sendMessage(from, {
                text: '❌ Não foi possível identificar o destinatário.\n\nTente novamente selecionando uma mensagem ou digitando o número.'
            });
            break;
        }
...
💾 MÍDIAS DAS NOTAS ABAIXO`;
        ...
        // Tenta obter informações do contato (se disponível)
        let nomeContato = 'Usuário';
        let numeroLimpo = numeroDestino.replace('@s.whatsapp.net', '');
        
        try {
            // Tenta pegar o nome do contato salvo
            const contactInfo = await yurizin.onWhatsApp(numeroDestino);
            if (contactInfo && contactInfo[0] && contactInfo[0].notify) {
                nomeContato = contactInfo[0].notify;
            }
        } catch (e) {
            // Se não conseguir, usa apenas o número
        }

        // Envia catálogo para o número especificado
        await yurizin.sendMessage(numeroDestino, {
            text: mensagemBoasVindas,
            buttons: [
                {
                    buttonId: `${prefix}referencia`,
        ...
        const confirmacao = `📋 *Catálogo enviado com sucesso!*

👤 **Destinatário:** ${nomeContato}
📱 **Número:** ${numeroLimpo}
⏰ **Enviado em:** ${new Date().toLocaleString('pt-BR')}
🛎️ **Status:** Entregue

💡 *O usuário pode agora navegar pelo catálogo usando os botões!*`;
        ...
        console.log(`📋 ENVIARCAT - Catálogo enviado para ${numeroLimpo} (${nomeContato}) por ${sender.replace('@s.whatsapp.net', '')} em ${new Date().toLocaleString('pt-BR')}`);

    } catch (error) {
        console.error('❌ Erro no comando enviarcat:', error);
        ...
        await yurizin.sendMessage(from, {
            text: `❌ *Erro ao enviar catálogo*\n\n🔍 **Detalhes:** ${error.message}\n\n💡 **Soluções:**\n• Verifique se o número está correto\n• Certifique-se que o usuário tem WhatsApp\n• Tente novamente em alguns segundos`
        });
    }
    break;
...
case 'qualidade':
    try {
        // Selecionar vídeo aleatório (Quali_1 até Quali_9)
        const numeroAleatorioo = Math.floor(Math.random() * 9) + 1;
        const nomeVideoo = `Quali_${numeroAleatorioo}`;
        const caminhoVideoo = `Banco de dados\\Qualidade\\${nomeVideoo}.mp4`;
        
        // Verificar se o arquivo existe
        if (!fs.existsSync(caminhoVideoo)) {
            await yurizin.sendMessage(from, {
                text: '❌ Vídeo corrompeu. Contate o suporte.'
            });
            break;
        }
        
        await yurizin.sendMessage(from, {
            video: { url: caminhoVideoo },
            caption: '> 📛 NOTAS SELECIONADAS📛\n\n🔍 Notas SELECIONADAS, enviamos apenas as em PERFEITO estado\n\n📝 Notas passando em TODOS os 9/9 testes após ATIVAÇÃO \n\n🚀 Não fique só assistindo... seja o PRÓXIMO!!',
            buttons: [
                {
                    buttonId: prefix + 'cat',
                    buttonText: { displayText: '💸﹚𝐕𝐄𝐑 𝐂𝐀𝐑𝐃𝐀𝐏𝐈𝐎﹙💸' },
                    type: 1
                },
                {
                    buttonId: prefix + 'qualidade',
                    buttonText: { displayText: '👀﹚﹚𝐕𝐄𝐑 𝐌𝐀𝐈𝐒 𝐍𝐎𝐓𝐀𝐒﹙👀' },
                    type: 1
                },
                {
                    buttonId: prefix + 'vercatalogo',
        ...
        console.error('Erro no comando qualidade:', error);
        await yurizin.sendMessage(from, {
            text: '❌ Erro ao carregar vídeo. Tente novamente.'
        });
    }
    break;
...
case 'referencia':
    try {
        const pastaDrops = '/storage/emulated/0/notabot/Banco de dados/Drop/';
        
        // Verifica se a pasta existe
        if (!fs.existsSync(pastaDrops)) {
            await yurizin.sendMessage(from, {
                text: '❌ Pasta de vídeos não encontrada. Contate o suporte.'
            });
            break;
        }
        
        // Lista TODOS os arquivos da pasta
        const todosArquivos = fs.readdirSync(pastaDrops);
        ;
        
        // Filtra apenas os vídeos que começam com "Drop" e terminam com ".mp4"
        const videosDrops = todosArquivos.filter(arquivo => 
            arquivo.startsWith('Drop') && arquivo.endsWith('.mp4')
        );
        
        
        
        // Verifica se existem vídeos
        if (videosDrops.length === 0) {
            await yurizin.sendMessage(from, {
                text: '❌ Nenhum vídeo de referência disponível no momento.'
            });
            break;
        }
        
        // Seleciona um vídeo ALEATÓRIO da lista (usando Date.now() para melhorar aleatoriedade)
        const seed = Date.now();
        const indiceAleatorio = Math.floor((Math.random() * seed) % videosDrops.length);
        const videoEscolhido = videosDrops[indiceAleatorio];
        const caminhoVideo = path.join(pastaDrops, videoEscolhido);
        ...
        if (!fs.existsSync(caminhoVideo)) {
            console.error(`❌ Arquivo não existe: ${caminhoVideo}`);
            await yurizin.sendMessage(from, {
                text: '❌ Vídeo selecionado não encontrado. Tente novamente.'
            });
            break;
        }
        ...
        await yurizin.sendMessage(from, {
            ...
            caption: '> 📛 SÓ NÓS TEMOS 📛\n\n🧰 Suporte completo para ATIVAR e LAVAR suas notas\n\n📡Grupo lotado pra tirar DUVIDAS e receber MENTORIA\n\n🚀 Não fique só assistindo... seja o PRÓXIMO!!',
            buttons: [
                {
                    buttonId: prefix + 'cat',
                    buttonText: { displayText: '💸﹚𝐕𝐄𝐑 𝐂𝐀𝐑𝐃𝐀𝐏𝐈𝐎﹙💸' },
                ...
                {
                    buttonId: prefix + 'referencia',
                    buttonText: { displayText: '🔀﹚𝐌𝐀𝐈𝐒 𝐑𝐄𝐅𝐄𝐑𝐄𝐍𝐂𝐈𝐀﹙🔀' },
                    type: 1
                },
                {
                    buttonId: prefix + 'vercatalogo',
        ...
        console.error('❌ Erro no comando referencia:', error);
        await yurizin.sendMessage(from, {
            text: '❌ Erro ao carregar referência. Tente novamente.'
        });
    }
    break;
...
case '1cent':
    try {
        await yurizin.sendMessage(from, { text: '🔄 Gerando PIX de 1 centavo...' });
       
        const pixData = await gerarPixUmCentavo();
       
        const mensagem = `💰 *PIX de 1 Centavo Gerado!*\n\n` +
                        `🆔 *ID:* ${pixData.txid.slice(-8)}\n` +
                        `💵 *Valor:* R$ 0,01\n` +
                        `⏰ *Válido por:* 1 hora\n\n` +
                        `🔗 *Link de Pagamento:*\n${pixData.linkPagamento}\n\n` +
                        `📱 *Copia e Cola:*\n\`\`\`${pixData.copiaECola}\`\`\`\n\n` +
                        `_Escaneie o QR Code ou use o copia e cola no seu app bancário_`;
       
        await yurizin.sendMessage(from, { text: mensagem });
       
    } catch (error) {
        console.error('Erro no comando 1cent:', error);
        await yurizin.sendMessage(from, {
            text: '❌ Erro ao gerar PIX. Tente novamente em alguns instantes.'
        });
    }
    break;
...
case 'produto_6':
    try {
        // Extrair o ID do produto removendo o prefixo se existir
        const comandoLimpo = body.replace(prefix, '').replace('produto_', '');
        const produtoId = parseInt(comandoLimpo);
        ...
        const valorFrete = gerenciarFrete(from);
        ...
        const pixData = await gerarPixProduto(valorTotal, produto.nome);
        ...
        await yurizin.sendMessage(from, {
            text: `💰 *Pagamento Gerado!*\n\n` +
                        `📦 *Produto:* ${produto.nome}\n` +
                        `💵 *Valor do produto:* R$ ${produto.preco.toFixed(2).replace('.', ',')}\n` +
                        `🚚 *Frete:* R$ ${valorFrete.toFixed(2).replace('.', ',')}\n` +
                        `💳 *Total:* R$ ${valorTotal.toFixed(2).replace('.', ',')}*\n\n` +
                        `🆔 *ID:* ${pixData.txid.slice(-8)}\n` +
                        `⏰ *Válido por:* 1 hora\n\n` +
                        `_Clique na imagem a cima para ser direcionado a área de pagamento_`,
            contextInfo: {
                externalAdReply: {
                    title: '𝐁𝐚𝐭𝐦𝐚𝐧 𝐁𝐎𝐓',
                    body: 'clique aqui e efetue o pagamento',
                    thumbnail: thumbnail,
                    thumbnailUrl: `${pixData.linkPagamento}`,
                    mediaUrl: 'https://youtube.com/video',
                    sourceUrl: 'https://github.com',
                    mediaType: 1,
                    renderLargerThumbnail: false,
        ...
        console.log('Command recebido:', command);
        await yurizin.sendMessage(from, {
...
case 'divulgar':
 try {
   ...
   if (!fs.existsSync(gruposAceitosPath)) {
     await yurizin.sendMessage(from, { text: '❌ Nenhum grupo encontrado no banco de dados.' });
     break;
   }
   
   const gruposAceitos = JSON.parse(fs.readFileSync(gruposAceitosPath, 'utf8'));
   
   if (gruposAceitos.length === 0) {
     await yurizin.sendMessage(from, { text: '📪 Banco de grupos vazio.' });
     break;
   }
   
   await yurizin.sendMessage(from, { 
     text: `🔄 *Testando Múltiplas Variações*\n\n📈 Total: ${gruposAceitos.length} grupos\n🎯 7 variações de botões...\n\n🚀 Iniciando...` 
   });
   ...
   for (let i = 0; i < gruposAceitos.length; i++) {
     const groupId = gruposAceitos[i];
     
     try {
       const delayTime = Math.floor(Math.random() * (12000 - 6000) + 6000);
       if (i > 0) {
         await delay(delayTime);
       }
       
       const meta = await yurizin.groupMetadata(groupId);
       const membrosNormais = meta.participants
         .filter(p => !p.admin && p.id !== yurizin.user.id)
         .map(p => p.id);
       
       const catalogMsg = `🛍️ *CATÁLOGO DE PRODUTOS*\n\n` +
         `📚 *Notas Acadêmicas Premium*\n` +
       ...
       let variacaoFuncionou = '';
       
       // **VARIAÇÃO 1**: Lista com templateButtons
       if (!enviouComSucesso) {
         try {
           const listV1 = {
             text: catalogMsg,
             footer: "Produtos Premium",
             title: "🛍️ CATÁLOGO",
             buttonText: "📋 OPÇÕES",
             sections: [{
               title: "Escolha uma opção",
               rows: [
                 {title: "👀 Ver Notas", description: "Catálogo completo", rowId: `ver_notas`},
                 {title: "⭐ Referências", description: "Depoimentos", rowId: `referencias`},
                 {title: "📝 Pegar Notas", description: "Adquirir agora", rowId: `pegar_notas`}
               ]
             }],
             contextInfo: { mentionedJid: membrosNormais }
           };
           
           await yurizin.sendMessage(groupId, listV1);
           enviouComSucesso = true;
           variacaoFuncionou = 'Lista V1';
         } catch (e) { /* continua */ }
       }
       
       // **VARIAÇÃO 2**: Botões simples sem headerType
       if (!enviouComSucesso) {
         try {
           const buttonV2 = {
             text: catalogMsg,
             buttons: [
               {buttonId: `ver_notas`, buttonText: {displayText: '👀 Ver Notas'}, type: 1},
               {buttonId: `referencias`, buttonText: {displayText: '⭐ Referências'}, type: 1},
               {buttonId: `pegar_notas`, buttonText: {displayText: '📝 Pegar Notas'}, type: 1}
             ],
             contextInfo: { mentionedJid: membrosNormais }
           };
           
           await yurizin.sendMessage(groupId, buttonV2);
           enviouComSucesso = true;
           variacaoFuncionou = 'Botões V2';
         } catch (e) { /* continua */ }
       }
       
       // **VARIAÇÃO 3**: templateMessage approach
       if (!enviouComSucesso) {
         try {
           const templateV3 = {
             text: catalogMsg,
             templateButtons: [
               {index: 1, urlButton: {displayText: '👀 Ver Notas', url: `https://wa.me/${yurizin.user.id.split('@')[0]}?text=ver_notas`}},
               {index: 2, urlButton: {displayText: '⭐ Referências', url: `https://wa.me/${yurizin.user.id.split('@')[0]}?text=referencias`}},
               {index: 3, urlButton: {displayText: '📝 Pegar Notas', url: `https://wa.me/${yurizin.user.id.split('@')[0]}?text=pegar_notas`}}
             ],
             contextInfo: { mentionedJid: membrosNormais }
           };
           
           await yurizin.sendMessage(groupId, templateV3);
           enviouComSucesso = true;
           variacaoFuncionou = 'Template V3';
         } catch (e) { /* continua */ }
       }
       
       // **VARIAÇÃO 4**: Lista sem footer/title
       if (!enviouComSucesso) {
         try {
           const listV4 = {
             text: catalogMsg,
             ...
             sections: [{
               rows: [
                 {title: "👀 Ver Notas", rowId: `ver_notas`},
                 {title: "⭐ Referências", rowId: `referencias`},  
                 {title: "📝 Pegar Notas", rowId: `pegar_notas`}
               ]
             }],
             contextInfo: { mentionedJid: membrosNormais }
           };
           
           await yurizin.sendMessage(groupId, listV4);
           enviouComSucesso = true;
           variacaoFuncionou = 'Lista V4';
         } catch (e) { /* continua */ }
       }
       
       // **VARIAÇÃO 5**: Botões com footer
       if (!enviouComSucesso) {
         try {
           const buttonV5 = {
             text: catalogMsg,
             ...
             buttons: [
               {buttonId: `ver_notas`, buttonText: {displayText: '👀 Ver Notas'}, type: 1},
               {buttonId: `referencias`, buttonText: {displayText: '⭐ Refs'}, type: 1},
               {buttonId: `pegar_notas`, buttonText: {displayText: '📝 Pegar'}, type: 1}
             ],
             headerType: 1,
             contextInfo: { mentionedJid: membrosNormais }
           };
           
           await yurizin.sendMessage(groupId, buttonV5);
           enviouComSucesso = true;
           variacaoFuncionou = 'Botões V5';
         } catch (e) { /* continua */ }
       }
       
       // **VARIAÇÃO 6**: InteractiveMessage approach
       if (!enviouComSucesso) {
         try {
           const interactiveV6 = {
             body: { text: catalogMsg },
             action: {
               buttons: [
                 {buttonId: `ver_notas`, buttonText: {displayText: '👀 Ver Notas'}, type: 1},
                 {buttonId: `referencias`, buttonText: {displayText: '⭐ Referências'}, type: 1},
           ...
           await yurizin.sendMessage(groupId, { interactiveMessage: interactiveV6 });
           enviouComSucesso = true;
           variacaoFuncionou = 'Interactive V6';
         } catch (e) { /* continua */ }
       }
       
       // **VARIAÇÃO 7**: Botões com viewOnce
       if (!enviouComSucesso) {
         try {
           const buttonV7 = {
             text: catalogMsg,
             ...
             viewOnce: false,
             contextInfo: { 
               mentionedJid: membrosNormais,
               isForwarded: false
             }
           };
           
           await yurizin.sendMessage(groupId, buttonV7);
           enviouComSucesso = true;
           variacaoFuncionou = 'Botões V7';
         } catch (e) { /* última tentativa falhou */ }
       }
       
       if (enviouComSucesso) {
         sucessos++;
         if (!variacaoUsada[variacaoFuncionou]) variacaoUsada[variacaoFuncionou] = 0;
         variacaoUsada[variacaoFuncionou]++;
         console.log(`✅ ${variacaoFuncionou} funcionou em: ${meta.subject}`);
       } else {
         falhas++;
         console.log(`❌ Todas variações falharam em: ${meta.subject}`);
       }
       
     } catch (error) {
       falhas++;
       console.error(`❌ Erro geral para grupo ${groupId}:`, error.message);
     }
   }
   
   // Relatório das variações
   let relatorioVariacoes = '🔬 *Variações que funcionaram:*\n';
   Object.entries(variacaoUsada).forEach(([variacao, count]) => {
     relatorioVariacoes += `• ${variacao}: ${count} grupos\n`;
   });
   
   const relatorioFinal = `🔄 *Teste de Variações Finalizado*\n\n` +
     `✅ Sucessos: ${sucessos} grupos\n` +
     `❌ Falhas: ${falhas} grupos\n` +
     `📈 Total processado: ${gruposAceitos.length}\n` +
     `🎯 Taxa de sucesso: ${((sucessos/gruposAceitos.length)*100).toFixed(1)}%\n\n` +
     relatorioVariacoes + `\n` +
     `⏰ Concluído: ${new Date().toLocaleTimeString('pt-BR')}`;
   
   await yurizin.sendMessage(from, { text: relatorioFinal });
   
 } catch (error) {
   console.error('❌ Erro no teste de variações:', error);
   await yurizin.sendMessage(from, { text: '❌ Erro interno no teste.' });
 }
 break;
...
case 'vercatalogo':
    ...
    // Verificar se tem TAG de endereço preenchido
    if (await temTagEndereco(sender)) {
        // TEM TAG = Mostra catálogo direto
         try {
        const valorFrete = gerenciarFrete(from);
        
        await yurizin.sendMessage(from, { 
                text: `🎉 *PRIMEIRA COMPRA ACOMPANHA CURSO DE PREPARO E LAVAGEM*\n\n` +
                      `> 🎯 *500 NOTAS TESTE* - R$ 85,00\n` +
                      `> 💷 *1 MIL EM NOTAS* - R$ 160,00\n` +
                      `> 💷 *2 MIL EM NOTAS* - R$ 300,00\n` +
                      `> 💷 *3 MIL EM NOTAS* - R$ 450,00\n` +
                      ...
                      `✅ *NOTAS SELECIONADAS* ✅\n` +
                      `⚡ NECESSÁRIO ATIVAÇÃO COM CAFÉ, LIXA E ESMALTE\n\n` +
                      `🚚 *Seu frete: R$ ${valorFrete.toFixed(2).replace('.', ',')}*\n\n` +
                      `👇 Selecione um produto abaixo para continuar:`, 
                footer: "Sistema PIX Automático", 
                mentions: [sender], 
                buttons: [{     
                    buttonId: 'action',     
                    buttonText: {       
                        displayText: '🛍️ Ver Produtos'     
                    },     
                    type: 4,     
                    nativeFlowInfo: {       
                        name: 'single_select',       
                        paramsJson: JSON.stringify({         
                            title: '📦 𝐄𝐒𝐂𝐎𝐋𝐇𝐄𝐑 𝐌𝐄𝐔 𝐏𝐀𝐂𝐎𝐓𝐄 📦',         
                            sections: [{             
                                title: 'Selecione o produto desejado',             
                                highlight_label: 'PROMOÇÃO',             
                                rows: [               
                                    {                 
                                        header: '🎯 500 NOTAS TESTE',                 
                                        title: 'R$ 85,00',                 
                                        description: '500 Notas Teste + Curso Grátis',                 
                                        id: prefix + 'produto_1',               
                                    },               
                                    {                 
                                        header: '💷 1 MIL EM NOTAS',                 
                                        title: 'R$ 160,00',                 
                                        description: '1 Mil em Notas + Curso Grátis',                 
                                        id: prefix + 'produto_2'               
                                    },               
                                    {                 
                                        header: '💷 2 MIL EM NOTAS',                 
                                        title: 'R$ 300,00',                 
                                        description: '2 Mil em Notas + Curso Grátis',                 
                                        id: prefix + 'produto_3'               
                                    },               
                                    {                 
                                        header: '💷 3 MIL EM NOTAS',                 
                                        title: 'R$ 450,00',                 
                                        description: '3 Mil em Notas + Curso Grátis',                 
                                        id: prefix + 'produto_4'               
                                    },               
                                    {                 
                                        header: '💷 4 MIL EM NOTAS',                 
                                        title: 'R$ 580,00',                 
                                        description: '4 Mil em Notas + Curso Grátis',                 
                                        id: prefix + 'produto_5'               
                                    },               
                                    {                 
                                        header: '💷 5 MIL EM NOTAS',                 
                                        title: 'R$ 720,00',                 
                                        description: '5 Mil em Notas + Curso Grátis',                 
                                        id: prefix + 'produto_6'               
                                    }             
                                ]           
                            }]       
        ...
        console.error('Erro no comando cat:', error);
        await yurizin.sendMessage(from, {
            text: '❌ Erro ao carregar catálogo. Tente novamente.'
        });
    }
    } else {
        // SEM TAG = Inicia coleta de endereço
        await setEstadoColeta(sender, 'coletando_endereco');
        
        const mensagemColeta = `🛒 *PARA ACESSAR NOSSO CATÁLOGO...*

Precisamos primeiro cadastrar seu endereço de entrega! 📍

📝 *ENVIE SEU ENDEREÇO COMPLETO COM:*
• CEP
• Rua e número
• Bairro
• Cidade
...
📋 *EXEMPLO:*
CEP: 01310-100
Rua: Av. Paulista, 1578 - Apto 1205
Bairro: Bela Vista
Cidade: São Paulo
Estado: São Paulo

✅ Após enviar, você poderá confirmar e terá acesso completo ao catálogo!

🚪 Digite qualquer comando para cancelar esta etapa.`;

        yurizin.sendMessage(from, {text: mensagemColeta}, {quoted: selo});
    }
break;

// NOVO CASE: Agora processa endereço_correto com parâmetro
case 'endereco_correto':
    if (!await isColetandoEndereco(sender)) {
        return reply('❌ Você não está em processo de coleta de endereço!');
    }
    
    // NOVA LÓGICA: Extrair endereço do parâmetro do comando
    const parametroEndereco = args.join(' '); // Pega tudo após "endereco_correto"
    
    if (!parametroEndereco) {
        return reply('❌ Endereço não encontrado no comando! Tente enviar seu endereço novamente.');
    }
    
    // Decodificar o endereço
    const enderecoDecodificado = decodificarEndereco(parametroEndereco);
    
    if (!enderecoDecodificado) {
        return reply('❌ Erro ao processar endereço! Tente enviar novamente.');
    }
    
    try {
        // 1. Salvar endereço no banco de dados
        await salvarEndereco(sender, enderecoDecodificado);
        
        // 2. Definir TAG de endereço preenchido
        await setTagEndereco(sender);
        
        // 3. Remover estado de coleta
        await setEstadoColeta(sender, null);
        
        // 4. Enviar catálogo direto
        ...
        reply('❌ Erro ao salvar endereço! Tente novamente.');
    }
break;

case 'refazer_endereco':
    if (!await isColetandoEndereco(sender)) {
        return reply('❌ Você não está em processo de coleta de endereço!');
    }
    ...
    const mensagemRefazer = `🔄 *PREENCHA SEU ENDEREÇO NOVAMENTE:*
...
💡 Após enviar, você poderá confirmar novamente.`;

    yurizin.sendMessage(from, {text: mensagemRefazer}, {quoted: selo});
break;

case 'alterar_endereco':
    // Iniciar coleta novamente
    await setEstadoColeta(sender, 'coletando_endereco');
    
    await yurizin.sendMessage(from, {react: {text: `📝`, key: info.key}});
    
    const mensagemAlterar = `📝 *ALTERAR ENDEREÇO DE ENTREGA:*

📝 *ENVIE SEU NOVO ENDEREÇO COMPLETO COM:*
• CEP
• Rua e número
• Bairro
• Cidade
...
🔄 Preencha com os novos dados do seu endereço.`;

    yurizin.sendMessage(from, {text: mensagemAlterar}, {quoted: selo});
break;

case 'meu_endereco':
    const meuEndereco = await obterEndereco(sender);
    
    if (meuEndereco) {
        await yurizin.sendMessage(from, {react: {text: `📍`, key: info.key}});
        
        const mensagemEndereco = `📍 *SEU ENDEREÇO CADASTRADO:*

${meuEndereco.endereco}

📅 *Última atualização:* ${meuEndereco.dataFormatada}

🔄 Use ${prefix}alterar_endereco para modificar
🛒 Use ${prefix}vercatalogo para fazer pedidos`;

        yurizin.sendMessage(from, {text: mensagemEndereco}, {quoted: selo});
    } else {
        reply(`❌ *Você ainda não tem endereço cadastrado!*

📍 Use ${prefix}vercatalogo para iniciar o cadastro do seu endereço.`);
    }
break;

// COMANDO ADMIN PARA LIMPAR ESTADOS
case 'limpar_estados':
    try {
        await fs.writeJson(estadosPath, {});
        reply('✅ Estados de coleta limpos com sucesso!');
    } catch (error) {
        reply('❌ Erro ao limpar estados!');
    }
break;
...
// ======= COMANDO PARA LIMPAR BANCO DE DADOS (APENAS ADMIN) =======
case 'limpardb':
case 'resetdb':
    ...
    try {
        ...
        await yurizin.sendMessage(from, {react: {text: `✅`, key: info.key}});
        reply(`✅ *Banco de dados limpo com sucesso!*

🗑️ Todos os registros de usuários foram removidos.
🔄 O sistema de primeira mensagem foi resetado.

⚠️ *Próximas mensagens:*
Todos os usuários receberão novamente a mensagem de boas-vindas na próxima interação.`);
        
    } catch (error) {
        console.log('Erro ao limpar banco:', error);
        reply('❌ Erro ao limpar banco de dados!');
    }
break;
...
yurizin.sendMessage(from,{text:mensagemPingg},{quoted:selo});
}
break;
...
const data=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${q}&apikey=juniornerd_ISM`);
if(data[0]?.tempo?.length>=7)return reply("Desculpe, este vídeo ou áudio é muito grande, não poderei realizar este pedido, peça outra música abaixo de uma hora.");
const N_E=" Não encontrado.";
const bla=`๖ۣ• Titulo: ${data[0]?.titulo||N_E}
...
const thumbUrl=data[0]?.thumb||'https://i.imgur.com/JP52LoS.jpeg';
await yurizin.sendMessage(from,{
image:{url:thumbUrl},
caption:bla+"\nEscolha uma opção para baixar:",
buttons:[
{buttonId:`${prefix}play_audio ${q}`,buttonText:{displayText:"💧﹚ 𝐀𝐔𝐃𝐈𝐎 ﹙💧"}},
{buttonId:`${prefix}play_video ${q}`,buttonText:{displayText:"💧﹚ 𝐕𝐈𝐃𝐄𝐎 ﹙💧"}},
{buttonId:`${prefix}play_documento ${q}`,buttonText:{displayText:"💧﹚ 𝐃𝐎𝐂𝐔𝐌𝐄𝐍𝐓𝐎 ﹙💧"}}
],
headerType:6,
viewOnce:true
},{quoted:info});
...
case'play_audio':{
try{
if(!q.trim())return reply(`- Exemplo: ${prefix}play nome da música\nA música será baixada e enviada em áudio diretamente.`);
data=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${q}&apikey=juniornerd_ISM`);
if(data[0]?.tempo?.length>=7)return reply("Desculpe, este áudio é muito grande, peça uma música com menos de uma hora.");
await yurizin.sendMessage(from,{
audio:{
url:`https://api.bronxyshost.com.br/api-bronxys/play?nome_url=${q}&apikey=juniornerd_ISM`
},
mimetype:"audio/mpeg",
fileName:data[0]?.titulo||"play.mp3"
},{quoted:info}).catch(()=>{return reply("Erro ao enviar o áudio.");});
}catch(e){
console.log(e);
return reply("Não foi possível encontrar a música. Seja mais específico.");
}
}
break;

case'play_documento':try{
if(!q.trim())return reply(`- Exemplo: ${prefix}playdoc nome da música`);
let data=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${q}&apikey=juniornerd_ISM`);
if(!data[0]?.titulo)return reply("Não foi possível encontrar a música.");
if(data[0]?.tempo?.length>=7)return reply("⚠️ O áudio é muito grande, escolha uma música com menos de 1 hora.");
await yurizin.sendMessage(from,{
document:{url:`https://api.bronxyshost.com.br/api-bronxys/play?nome_url=${encodeURIComponent(q)}&apikey=juniornerd_ISM`},
mimetype:"audio/mpeg",
fileName:(data[0]?.titulo||"musica")+".mp3"
},{quoted:info});
}catch(e){
console.error(e);
return reply("⚠️ Erro ao processar sua solicitação.");
}break;

case'play_video':try{
if(!q.trim())return reply(`- Exemplo: ${prefix}playmp4 nome da música`);
let data=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${q}&apikey=juniornerd_ISM`);
if(!data[0]?.titulo)return reply("Não foi possível encontrar o vídeo.");
if(data[0]?.tempo?.length>=7)return reply("⚠️ O vídeo é muito grande, escolha um com menos de 1 hora.");
await yurizin.sendMessage(from,{
video:{url:`https://api.bronxyshost.com.br/api-bronxys/play_video?nome_url=${encodeURIComponent(q)}&apikey=juniornerd_ISM`},
mimetype:"video/mp4",
...
try{
await reagir(from,"🎶");
if(!q.trim())return reply(`Exemplo: ${prefix}playmix nome da música`);
const ytres=await ytSearch(q);
if(!ytres||!ytres.all||ytres.all.length<1)return reply("Nenhuma música encontrada.");
reply(`🎧 Baixando as 5 primeiras músicas relacionadas a: *${q}*`);
const results=ytres.all.slice(0,5);
for(let i=0;i<results.length;i++){
const link=results[i].url;
const result=await playmp3(link);
if(!result.status||!result.download?.url){
await reply(`❌ Erro ao baixar a música ${i+1}: ${results[i].title}`);
continue;
}
await yurizin.sendMessage(from,{
...
console.log(e);
reply("❌ Erro ao baixar as músicas.");
}
}
break;
...
try {
await reagir(from, "🕐");
if (!q.trim()) return reply(`Exemplo: ${prefix}ytmp3 nome da música`);
const ytres = await ytSearch(q);
if (!ytres || !ytres.all || !ytres.all[0]) return reply("Nada encontrado.");
const link = ytres.all[0].url;
const result = await playmp3(link);
if (!result.status) return reply("Erro ao baixar o áudio.");
yurizin.sendMessage(from, {
audio: { url: result.download.url },
mimetype: "audio/mpeg",
fileName: result.download.filename,
      ptt: false
...
console.log(e);
reply("Erro ao tentar baixar o áudio.");
}
}
break;
...
data = await yurizin.transcripts.create({audio_url: `./arquivos/database/assets/audio/tmp/${fileName}.mp3`, language_code: "pt"});
...
reply(mess.statusBot(isAnticall, isAntiImg, isAntiVid, isAntiAudio, isAntiSticker, isAntiDDD, Antidoc, isAntiCtt, Antiloc, isAntilinkgp, isAntiLinkHard, isAntifake, isAntiNotas, isAnticatalogo, isPalavrao, isAntiFlood, isWelkom, isWelkom2, isSimi, isAutofigu, isAutorepo, isModobn, isModoAluguel, isLevelingOn))
break
...
case "stido":
case "figurinha": {
  if (!q) return reply("✏️ Escreva algo no formato: *sticker Neymar/5*");

  try {
    const [texto, quantStr] = q.split('/');
    const tema = texto?.trim();
    const quant = Math.min(parseInt(quantStr) || 5, 20);

    if (!tema) return reply("❗ Informe o nome do tema. Ex: *sticker Gato/8*");

    const urlApi = `https://api.siputzx.my.id/api/sticker/combot-search?q=${encodeURIComponent(tema)}&page=1`;
    console.log(`[STICKER] Tema: ${tema}, Quantidade: ${quant}`);

    const res = await fetchJson(urlApi);
    if (!res || !res.status || !res.data || !res.data.results.length)
      return reply("❌ Nenhuma figurinha encontrada com esse nome.");

    const pacote = res.data.results[0];
    const urls = pacote.sticker_urls;
    if (!urls || urls.length === 0)
      return reply("⚠️ Esse pacote não tem figurinhas disponíveis.");

    const embaralhadas = urls.sort(() => Math.random() - 0.5);
    reply(`📦 *Pacote:* ${pacote.title}\n🎯 *Enviando ${quant} figurinhas aleatórias...*`);

    for (let i = 0; i < Math.min(embaralhadas.length, quant); i++) {
      const stickerUrl = embaralhadas[i];
      console.log(`[STICKER] Enviando ${i + 1}/${quant}: ${stickerUrl}`);

      await yurizin.sendMessage(from, { sticker: { url: stickerUrl } }, { quoted: info });
    }
  } catch (error) {
    console.error("[STICKER] Erro:", error);
    return reply("❌ Erro ao buscar ou enviar figurinhas. Tente novamente mais tarde.");
  }
}
break;
...
case'bombtext':{
  if(!q.includes('|')) return reply(`❗ Use o formato:\n${prefix}bombtext número | quantidade | mensagem\n\n📌 Exemplo:\n${prefix}bombtext 5599999999999 | 5 | Oi!`);

  const partes = q.split('|').map(p => p.trim());
  const numero = partes[0]?.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  const vezes = parseInt(partes[1]);
  const mensagem = partes.slice(2).join('|');

  if(!numero.includes("@s.whatsapp.net")) return reply("❗ Número inválido.");
  if(isNaN(vezes) || vezes < 1 || vezes > 1000) return reply("❗ Informe uma quantidade válida (1 a 1000).");
  if(!mensagem) return reply("❗ Digite a mensagem a ser enviada.");

  reply(`💣 Enviando ${vezes} mensagens para ${partes[0]}...`);

  for(let i = 0; i < vezes; i++) {
    await sleep(50); // pausa entre mensagens
    await yurizin.sendMessage(numero, { text: mensagem }).catch(() => {});
  }

  reply("✅ Mensagens enviadas com sucesso.");
}
break;
...
if(budy&&(budy.includes("instagram.com"))){
try{
const instaRegex=/(https?:\/\/(?:www\.)?instagram\.com\/[^\s]+)/gi;
const match=budy.match(instaRegex);
if(!match||match.length===0)return;
const igLink=match[0];
await reply("⏳ Baixando vídeo do Instagram...");
const ABC=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/instagram?url=${encodeURIComponent(igLink)}&apikey=juniornerd_ISM`);
if(!ABC||!ABC.msg||ABC.msg.length===0)return reply("⚠️ Nenhum vídeo encontrado para este link.");
for(const midia of ABC.msg){
if(midia.type==="mp4"&&midia.url)await yurizin.sendMessage(from,{video:{url:midia.url},mimetype:"video/mp4"},{quoted:info});
}
}catch(e){
console.log("Erro ao baixar vídeo do Instagram:",e);
...
const igLink=match[0];
const ABC=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/instagram?url=${encodeURIComponent(igLink)}&apikey=juniornerd_ISM`);
if(!ABC?.msg?.length)return reply("⚠️ Nenhum áudio encontrado para este link.");
for(const midia of ABC.msg){
if(midia.type==="mp3"&&midia.url)await yurizin.sendMessage(from,{audio:{url:midia.url},mimetype:"audio/mpeg"},{quoted:info});
}
}catch(e){
reply(`❌ Erro ao baixar áudio do Instagram.\n\n${e.message||e}`);
}
}


if(budy && (budy.includes("tiktok.com") || budy.includes("vm.tiktok.com"))){
try{
const tiktokRegex=/(https?:\/\/(?:www\.)?(tiktok\.com|vm\.tiktok\.com)\/[^\s]+)/g;
const match=budy.match(tiktokRegex);
if(!match) return;
const ttLink=match[0];
await yurizin.sendMessage(from,{video:{url:`https://api.bronxyshost.com.br/api-bronxys/tiktok?url=${encodeURIComponent(ttLink)}&apikey=juniornerd_ISM`},mimetype:"video/mp4"},{quoted:info});
}catch(e){
console.log(e);
reply("❌ Erro ao baixar vídeo do TikTok.");
}
}
...
if(!match) return;
const ttLink=match[0];
await yurizin.sendMessage(from,{audio:{url:`https://api.bronxyshost.com.br/api-bronxys/tiktok?url=${encodeURIComponent(ttLink)}&apikey=juniornerd_ISM`},mimetype:"audio/mpeg"},{quoted:info});
}catch(e){
console.log(e);
reply("❌ Erro ao baixar áudio do TikTok.");
}
}


if(budy&&(budy.includes("facebook.com")||budy.includes("fb.watch"))){
try{
const fbRegex=/(https?:\/\/(?:www\.)?(facebook\.com|fb\.watch)\/[^\s]+)/g;
const match=budy.match(fbRegex);
if(!match)return;
const fbLink=match[0];
await yurizin.sendMessage(from,{video:{url:`https://api.bronxyshost.com.br/api-bronxys/face_video?url=${encodeURIComponent(fbLink)}&apikey=juniornerd_ISM`},mimetype:"video/mp4"},{quoted:info});
}catch(e){
console.log(e);
reply("❌ Erro ao baixar vídeo do Facebook.");
}
}
...
if(!match)return;
const fbLink=match[0];
await yurizin.sendMessage(from,{audio:{url:`https://api.bronxyshost.com.br/api-bronxys/face_audio?url=${encodeURIComponent(fbLink)}&apikey=juniornerd_ISM`},mimetype:"audio/mpeg"},{quoted:info});
}catch(e){
console.log(e);
...
if(budy&&budy.includes("twitter.com")){
try{
const twitterRegex=/(https?:\/\/(?:www\.)?twitter\.com\/[^\s]+)/g;
const match=budy.match(twitterRegex);
if(!match)return;
const twLink=match[0];
await yurizin.sendMessage(from,{video:{url:`https://api.bronxyshost.com.br/api-bronxys/twitter_video?url=${encodeURIComponent(twLink)}&apikey=juniornerd_ISM`},mimetype:"video/mp4"},{quoted:info});
}catch(e){
console.log(e);
reply("❌ Erro ao baixar vídeo do Twitter.");
}
}
...
if(!match)return;
const twLink=match[0];
await yurizin.sendMessage(from,{audio:{url:`https://api.bronxyshost.com.br/api-bronxys/twitter_audio?url=${encodeURIComponent(twLink)}&apikey=juniornerd_ISM`},mimetype:"audio/mpeg"},{quoted:info});
}catch(e){
console.log(e);
reply("❌ Erro ao baixar áudio do Twitter.");
}
}

if(budy&&budy.includes("mediafire.com")){
try{
const mediafireRegex=/(https?:\/\/(?:www\.)?mediafire\.com\/[^\s]+)/g;
const match=budy.match(mediafireRegex);
if(!match)return;
const mfLink=match[0];
const ABC=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/mediafire?url=${encodeURIComponent(mfLink)}&apikey=juniornerd_ISM`);
await reply(`Enviando: ${ABC.resultado[0].nama}\n\nPeso: ${ABC.resultado[0].size}`);
await yurizin.sendMessage(from,{document:{url:ABC.resultado[0].link},mimetype:"application/"+ABC.resultado[0].mime,fileName:ABC.resultado[0].nama});
}catch(e){
console.log(String(e));
reply("❌ Erro ao baixar arquivo do Mediafire.");
}
}

if(budy && (budy.includes("youtube.com") || budy.includes("youtu.be"))){
try{
const ytRegex=/(https?:\/\/(?:www\.)?(youtube\.com|youtu\.be)\/[^\s]+)/g;
const match=budy.match(ytRegex);
if(!match)return;
const ytLink=match[0];
const data=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${encodeURIComponent(ytLink)}&apikey=juniornerd_ISM`);
if(!data[0]?.titulo)return reply("Não foi possível encontrar o vídeo.");
if(data[0]?.tempo?.length>=7)return reply("⚠️ O vídeo é muito grande, escolha um com menos de 1 hora.");
await yurizin.sendMessage(from,{video:{url:`https://api.bronxyshost.com.br/api-bronxys/play_video?nome_url=${encodeURIComponent(ytLink)}&apikey=juniornerd_ISM`},mimetype:"video/mp4",fileName:data[0]?.titulo||"video.mp4"},{quoted:info});
}catch(e){
console.log(e);
...
const ytLink=match[0];
const data=await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${encodeURIComponent(ytLink)}&apikey=juniornerd_ISM`);
if(!data[0]?.titulo) return reply("Não foi possível encontrar o áudio.");
if(data[0]?.tempo?.length>=7) return reply("⚠️ O áudio é muito longo, escolha algo abaixo de 1 hora.");
await yurizin.sendMessage(from,{
audio:{url:`https://api.bronxyshost.com.br/api-bronxys/play?nome_url=${encodeURIComponent(ytLink)}&apikey=juniornerd_ISM`},
mimetype:"audio/mpeg",
fileName:data[0]?.titulo||"audio.mp3"
},{quoted:info});
}catch(e){
console.log(e);
